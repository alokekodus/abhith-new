    ...
    $.ajax({
           ...
           var topic=result.result.lesson;
            ...
            <video id="player" class="video-js vjs-big-play-centered" controls preload="auto"
                    poster="${base_url}/${topic.lesson_attachment.video_thumbnail_image}" data-setup="{}">
                    <source src="${base_url}/${topic.lesson_attachment.attachment_origin_url}" type="video/mp4"
                        class="w100" />
                </video>`;
            ...
            lessons.forEach(function(lesson) {
              displayLessons += `
                                <div class="lesson-video-content d-flex mx-1">
                                  <p><div><img src="${base_url}/${lesson.lesson_attachment.video_thumbnail_image}" style="height:50px;width:70px;"></div><i class="fa fa-play-circle"></i> ${lesson.name} &nbsp;
